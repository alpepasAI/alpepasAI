# Hi there, I'm Álvaro Pérez Pastor 👋

[English Version 🇬🇧](#english-version) | [Versión en Español 🇪🇸](#versión-en-español)

---

## English Version

### Software Specialist | IT Application Owner | L3 Support Coordinator | AI R&D Explorer

Welcome to my GitHub space! I am a **Computer Engineer** with over 20 years of experience ensuring the reliability, security, and continuous operation of critical enterprise systems in the international banking sector (collaborating with institutions like Deutsche Bank and Banco Santander).

Currently, I combine my solid enterprise backend foundations with active research and hands-on development in **Artificial Intelligence, Agentic Architectures, and Modern Web/Android Solutions** through my personal laboratory, **alpepasLabs**.

### 🌐 alpepasLabs - R&D Projects

*   📚 **[Bibliotelia](https://github.com/alpepasAI/bibliotelia) — AI-Powered Book Cataloger & Social Network**
    *   *Description:* A book cataloger and social network for readers. It integrates a real-time book spine scanner using multimodal LLM APIs to automatically index books and extract metadata.
    *   *Tech Stack:* React, Supabase (Postgres & RLS Security), Sentry (Observability), Playwright (E2E Testing).
*   🍳 **[Cuinatelia](https://github.com/alpepasAI/cuinatelia) — Smart Multimodal Recipe Extractor**
    *   *Description:* A mobile-first Progressive Web App (PWA) that automatically extracts and structures recipes from web URLs, voice notes, screenshots, or camera video capture using LLM integrations.
    *   *Tech Stack:* React, TailwindCSS, Supabase Database, Postgres RLS, Multimodal LLM APIs.
*   🎮 **[InFunIt](https://github.com/alpepasAI/infunit) — Offline Local Multi-Device Android Game**
    *   *Description:* A native Android application for local social games that operates completely offline. It establishes direct multi-device communication using TCP sockets over Wi-Fi Direct and Bluetooth.
    *   *Tech Stack:* Android SDK (Java), TCP Sockets, Local Networking, Golden Testing.
*   📝 **[Prisma](https://github.com/alpepasAI/prisma) — Agentic Technical Blog Generator**
    *   *Description:* A lightweight blog platform built with vanilla ES6 JavaScript (zero external framework dependencies) that leverages AI agents to automatically generate multiformat technical posts optimized for SEO.
    *   *Tech Stack:* Vanilla JavaScript (ES6), HTML5/CSS3 Semantic layout, LLM API integration.

### 🛠️ Tech Stack & Skills

#### Modern Web & AI Integrations
![](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![](https://img.shields.io/badge/Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white)
![](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![](https://img.shields.io/badge/Android-3DDC84?style=flat-square&logo=android&logoColor=white)
![](https://img.shields.io/badge/TailwindCSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white)
![](https://img.shields.io/badge/Playwright-2EAD33?style=flat-square&logo=playwright&logoColor=white)

#### Enterprise Backend & Operations
![](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=spring&logoColor=white)
![](https://img.shields.io/badge/Oracle-F80000?style=flat-square&logo=oracle&logoColor=white)
![](https://img.shields.io/badge/Control--M-0083C4?style=flat-square&logo=bmc&logoColor=white)
![](https://img.shields.io/badge/OpenShift-EE0000?style=flat-square&logo=redhatopenshift&logoColor=white)
![](https://img.shields.io/badge/Linux_/_UNIX-FCC624?style=flat-square&logo=linux&logoColor=black)

### 💼 Core Enterprise Competencies
*   **IT Application Ownership (ITAO):** Regulatory compliance, IT security standards, and audits.
*   **Production Support (L3):** High-priority incident troubleshooting, root cause analysis, and batch flow monitorization.
*   **Release & Configuration Management:** Deployments, environment sync, and infrastructure coordination.
*   **Agile Methodologies:** Active contributor in scrum teams.
*   **Knowledge Transfer:** Successful coordination and hand-over to offshore teams in India.

### 📬 Connect with Me
*   **LinkedIn:** [Álvaro Pérez Pastor](https://www.linkedin.com/in/alvaro-perez-pastor)
*   **Portfolio:** [alpepasLabs](https://alpepaslabs.com)
*   **Email:** info@alpepaslabs.com

---

## Versión en Español

### Especialista de Software | IT Application Owner | Coordinador de Soporte L3 | Explorador I+D en IA

¡Bienvenido a mi espacio en GitHub! Soy **Ingeniero Informático** con más de 20 años de trayectoria garantizando la fiabilidad, seguridad y continuidad operativa de sistemas empresariales críticos en el sector bancario internacional (colaborando con entidades como Deutsche Bank y Banco Santander).

Actualmente, combino mi sólida base en backend corporativo con la investigación activa y el desarrollo práctico en **Inteligencia Artificial, Arquitecturas Agénticas y Soluciones Web/Android Modernas** a través de mi laboratorio personal, **alpepasLabs**.

### 🌐 alpepasLabs - Proyectos de I+D

*   📚 **[Bibliotelia](https://github.com/alpepasAI/bibliotelia) — Catalogador de Libros & Red Social Asistida por IA**
    *   *Descripción:* Red social y catalogador de libros personal. Integra un escáner de lomos en tiempo real utilizando APIs de LLM multimodales para indexar libros y extraer metadatos automáticamente.
    *   *Stack:* React, Supabase (Postgres y Seguridad RLS), Sentry (Observabilidad), Playwright (Tests E2E).
*   🍳 **[Cuinatelia](https://github.com/alpepasAI/cuinatelia) — Extractor Multimodal Inteligente de Recetas**
    *   *Descripción:* Progressive Web App (PWA) mobile-first que extrae y estructura recetas automáticamente a partir de URLs, notas de voz, capturas de pantalla o vídeo en vivo mediante integraciones de LLM.
    *   *Stack:* React, TailwindCSS, Supabase Database, Postgres RLS, APIs de LLM Multimodales.
*   🎮 **[InFunIt](https://github.com/alpepasAI/infunit) — Juego Android Multidispositivo Local Sin Conexión**
    *   *Descripción:* Aplicación nativa Android para juegos sociales presenciales que funciona 100% sin internet, mediante comunicación directa por sockets TCP sobre Wi-Fi Direct y Bluetooth.
    *   *Stack:* Android SDK (Java), Sockets TCP, Red Local, Golden Testing.
*   📝 **[Prisma](https://github.com/alpepasAI/prisma) — Generador Agéntico de Blogs Técnicos**
    *   *Descripción:* Plataforma ligera de blog técnico construida en JavaScript ES6 nativo (sin dependencias de frameworks) que emplea agentes de IA para la generación automática de posts multiformato optimizados para SEO.
    *   *Stack:* Vanilla JavaScript (ES6), Maquetación semántica HTML5/CSS3, integración de APIs de LLM.

### 🛠️ Tecnologías y Aptitudes

#### Web Moderna e Integraciones de IA
![](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![](https://img.shields.io/badge/Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white)
![](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![](https://img.shields.io/badge/Android-3DDC84?style=flat-square&logo=android&logoColor=white)
![](https://img.shields.io/badge/TailwindCSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white)
![](https://img.shields.io/badge/Playwright-2EAD33?style=flat-square&logo=playwright&logoColor=white)

#### Backend Empresarial y Operaciones
![](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=spring&logoColor=white)
![](https://img.shields.io/badge/Oracle-F80000?style=flat-square&logo=oracle&logoColor=white)
![](https://img.shields.io/badge/Control--M-0083C4?style=flat-square&logo=bmc&logoColor=white)
![](https://img.shields.io/badge/OpenShift-EE0000?style=flat-square&logo=redhatopenshift&logoColor=white)
![](https://img.shields.io/badge/Linux_/_UNIX-FCC624?style=flat-square&logo=linux&logoColor=black)

### 💼 Competencias Clave en la Empresa
*   **IT Application Ownership (ITAO):** Cumplimiento regulatorio, estándares de seguridad IT y auditorías físicas/lógicas.
*   **Soporte de Producción (L3):** Resolución de incidencias de alta prioridad, análisis de causa raíz y monitorización de flujos Batch.
*   **Gestión de Despliegues y Configuración (Release):** Instalaciones, sincronización de entornos y coordinación de infraestructura.
*   **Metodologías Ágiles:** Colaboración activa en equipos Scrum.
*   **Traspaso de Conocimiento:** Coordinación exitosa y hand-over a equipos offshore en India (Pune).

### 📬 Contacto
*   **LinkedIn:** [Álvaro Pérez Pastor](https://www.linkedin.com/in/alvaro-perez-pastor)
*   **Portfolio:** [alpepasLabs](https://alpepaslabs.com)
*   **Email:** info@alpepaslabs.com
